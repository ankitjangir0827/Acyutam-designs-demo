...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /**
   * GLOBAL XSS & CODE INJECTION PROTECTION ENGINE — ACHYUTAM BUILDER®
   * Ensures all user inputs are strictly safe plain-text strings.
   * Strips executable code, scripts, HTML tags, and dangerous characters across all forms.
   */
  window.sanitizeInputString = function(input) {
    if (input === null || input === undefined) return '';
    let clean = String(input);

    // 1. Remove script tags, iframe tags, style tags, link tags, and inline event handlers
    clean = clean
    ...
    // 2. Strip HTML tags entirely
    clean = clean.replace(/<\/?[^>]+(>|$)/g, '');

    // 3. Escape HTML entities to safe plain-text equivalents
    const htmlMap = {
      '&': '&amp;',
      '<': '&lt;',
      '>': '&gt;',
    ...
    return clean.replace(/[&<>"'/`]/g, (m) => htmlMap[m]).trim();
  };

  window.stripHTMLCode = function(input) {
    if (!input) return '';
    return String(input).replace(/<\/?[^>]+(>|$)/g, '').trim();
  };

  // Real-time live input cleaner on all text, email, tel, password inputs & textareas
  document.addEventListener("input", function(e) {
    const el = e.target;
    if (el && (el.tagName === "INPUT" || el.tagName === "TEXTAREA")) {
      const type = (el.type || "").toLowerCase();
      if (type !== "password" && type !== "file") {
        if (/<script|<iframe|javascript:|on\w+=/i.test(el.value)) {
          el.value = window.stripHTMLCode(el.value);
        }
      }
    }
  }, { capture: true });

  // Intercept all form submissions site-wide to sanitize all fields before dispatch
  document.addEventListener("submit", function(e) {
    ...
    if (form && form.elements) {
      for (let i = 0; i < form.elements.length; i++) {
        ...
        if (el && (el.tagName === "INPUT" || el.tagName === "TEXTAREA")) {
          const type = (el.type || "").toLowerCase();
          if (type !== "password" && type !== "file" && type !== "hidden" && type !== "submit") {
            el.value = window.stripHTMLCode(el.value);
          }
        }
      }
  ...
  console.log("🛡️ Achyutam Builder® Security Guard Active: Strict String Input & Anti-XSS Code Protection Online.");
});
